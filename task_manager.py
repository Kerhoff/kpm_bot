...
def get_new_cards_without_story() -> dict:
    ...
    cards_not_in_open_stories: list = [card for card in cards if card['desc'].split('/')[-1].split(')')[0] not in open_stories_keys]

    closed_stories_keys: set = set([story['key'] for story in closed_stories])
    NEW_CARDS = [card for card in cards_not_in_open_stories if card['desc'].split('/')[-1].split(')')[0] not in closed_stories_keys]
...
def create_new_stories_cards_on_board(new_stories=NEW_STORIES) -> dict:
    success = 0
    fail = 0
...
def delete_closed_stories_cards(completed_cards=COMPLETED_CARDS) -> dict:
    success: int = 0
    fail: int = 0
